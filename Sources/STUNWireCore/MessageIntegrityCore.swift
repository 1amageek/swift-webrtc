...
public enum MessageIntegrityCore: Sendable {

    /// HMAC-SHA1 length (the MESSAGE-INTEGRITY value is always 20 bytes).
    public static let macSize = 20
    ...
    public static func compute<M: MessageAuthenticationCode>(
        data: [UInt8],
        key: [UInt8],
        as macType: M.Type
    ) -> [UInt8] {
    ...
    ///   - key: the HMAC key.
    ///   - macType: the concrete MAC type backing the seam (HMAC-SHA1).
    public static func verifyWithResult<M: MessageAuthenticationCode>(
        message: [UInt8],
        key: [UInt8],
        as macType: M.Type
    ) -> IntegrityResult {
