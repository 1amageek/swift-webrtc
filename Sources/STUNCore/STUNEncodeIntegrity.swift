...
/// `MessageAuthenticationCode` seam (host: HMAC-SHA1 via swift-crypto; Embedded:
/// BoringSSL), so the same code signs identically on both builds.
///
...
extension STUNMessage {
    ...
    public func encodeWithIntegrityBytes(key: [UInt8]) -> [UInt8] {
