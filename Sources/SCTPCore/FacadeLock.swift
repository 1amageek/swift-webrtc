...
/// unavailable).
///
/// `SCTPAssociation` is a `final class & Sendable` adapter that holds its
/// Embedded-clean value-type engine (`SCTPAssociationEngine`) behind this lock so
/// its public methods stay `Sendable`-safe. The engine itself holds no lock; the
/// adapter serialises every mutation here.
///
