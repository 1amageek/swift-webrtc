...
struct STUNAttribute: Sendable, Equatable {
    /// Attribute type
    let type: UInt16
    ...
    func parseXorMappedAddress(transactionID: TransactionID) -> (address: [UInt8], port: UInt16)? {
    ...
    static func iceControlled(tiebreaker: UInt64) -> STUNAttribute {
        var data = [UInt8](repeating: 0, count: 8)
        for i in 0..<8 {
            data[i] = UInt8((tiebreaker >> (56 - i * 8)) & 0xFF)
        }
    ...
    static func errorCode(_ code: UInt16, reason: String) -> STUNAttribute {
