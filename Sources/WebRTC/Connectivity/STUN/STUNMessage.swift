...
struct STUNMessageType: Sendable, Equatable, Hashable {
    ...
    init(method: STUNMethod, class: STUNClass) {
    ...
    static let bindingRequest = STUNMessageType(method: .binding, class: .request)
    static let bindingSuccessResponse = STUNMessageType(method: .binding, class: .successResponse)
    static let bindingErrorResponse = STUNMessageType(method: .binding, class: .errorResponse)
    static let bindingIndication = STUNMessageType(method: .binding, class: .indication)
}
...
struct STUNMessage: Sendable {
    ...
    static func bindingSuccessResponse(
        transactionID: TransactionID,
        address: [UInt8],
        port: UInt16
    ) -> STUNMessage {
    ...
    static func bindingErrorResponse(
        transactionID: TransactionID,
        errorCode: UInt16,
        reason: String
    ) -> STUNMessage {
    ...
    func attribute(ofType type: STUNAttributeType) -> STUNAttribute? {
        attributes.first { $0.type == type.rawValue }
    }

    /// Check if this is a STUN message (first two bits are 0)
    static func isSTUN(_ data: [UInt8]) -> Bool {
    ...
    static func encodeAttributes(_ attributes: [STUNAttribute]) -> [UInt8] {
        var data = [UInt8]()
    ...
    static func decode(from data: [UInt8]) throws(STUNWireError) -> STUNMessage {
