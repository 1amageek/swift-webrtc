...
/// the same `Synchronization.Mutex` contract on every target), performs
...
final class ICELiteAgent: Sendable {
    ...
    init(credentials: ICECredentials = ICECredentials()) {
    ...
    func isPeerValidatedBytes(address: [UInt8], port: UInt16) -> Bool {
    ...
    func isPeerValidated(address: Data, port: UInt16) -> Bool {
